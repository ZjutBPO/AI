...
考虑到通勤用户和非通勤用户每日出行存在较大差异，需要根据地铁出行记录分别研究通勤用户和非通勤用户。本文根据不同用户的出行规律制定了区分通勤用户与非通勤用户的判决规则，判决流程图如图 3-4 所示，判决算法如算法 3-1所示。本规则首先分析用户出行数据推断用户的住宅站点和公司站点[69]，取出每名用户上午 10 点前记录的出发站点和下午 5 点后记录的到达站点作为候选住宅站点。若某候选住宅站点的出行频率大于阈值，则定义此站点为该用户的住宅站点。取前述记录中另一站点作为候选公司站点，根据阈值判断是否确定为该用户的公司站点。若没有超过阈值频率的站点，则判定为没有此站点。若用户存在明确的住宅站点和公司站点，定义为通勤用户，若仅存在住宅站点不存在公司站点，则定义为非通勤用户。除了通勤者和非通勤者之外，还存在少量出行习惯不固定
...
**原来打算使用马尔科夫模型进行出行预测但是因为人员出行记录太少，打算尝试使用LSTM进行出行预测**
