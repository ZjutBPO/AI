...
# 1 6 11 16 21
# 将数据格式化成监督学习型数据
reframed = series_to_supervised(MainStationData, n_steps, 1)
reframed = reframed.loc[(reframed["var2(t)"] >= TimeStart) & (reframed["var2(t)"] < TimeEnd)]
reframed = reframed.drop(["var2(t-{})".format(i+1) for i in range(n_steps)] + ["var2(t)"],axis = 1)

reframed = reframed.iloc[:,:n_steps * n_features + 1]
...
pyplot.legend()
pyplot.show()
# model.save("{}-minute forecast LSTM-Station{}.h5".format(TimeInterval,Station))